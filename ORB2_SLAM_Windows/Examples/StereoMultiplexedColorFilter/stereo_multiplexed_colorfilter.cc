...
int main(int argc, char **argv)
{
    if(argc != 5)
    {
        cerr << endl << "Usage: ./stereo_euroc path_to_vocabulary path_to_settings path_to_left_folder path_to_times_file" << endl;
        return 1;
    }
    ...
    LoadImages(string(argv[3]), string(argv[4]), vstrImageLeft, vTimeStamp);
    ...
    // Main loop
    cv::Mat imMulti,imLeft, imRight, imLeftRect, imRightRect;
    for(int ni=0; ni<nImages; ni++)
    {
        // Read left and right images from file
		imMulti = cv::imread(vstrImageLeft[ni],CV_LOAD_IMAGE_UNCHANGED);
		...
		RGB2gb_gray(imMulti, imRight);

		RGB2r_gray(imMulti, imLeft);


        //cv::remap(imLeft,imLeftRect,M1l,M2l,cv::INTER_LINEAR);
		//cv::remap(imRight, imRightRect, M1l, M2l, cv::INTER_LINEAR);
